...
| Four days of instruction and capstones | 24 instructional packages (`lib/coding-lessons/`) with sectioned reader UI, depth validation (`validateLessonPackageDepth`), six challenges, four runnable starter projects, a Day 2 FastAPI service with `/health`, `/triage`, and `/rules`, a Day 3 AI-maintenance-triage project with covered configuration/schema/unit/input/injection failure cases, and a Day 4 handoff capstone. Lessons teach; labs prove. | Implemented for local pilot. |
...
| Inside-the-machine simulator | Server-reviewed command-path reconstruction and explanation of keyboard → shell → operating system → Python → program → output | Implemented as a deterministic beginner simulation; it is a conceptual mental-model exercise, not a system trace. |
...
| Test repair simulator | Protected test-quality analysis covering empty assertions, wrong requirements, unstable dependencies, private-detail coupling, and missing exact boundaries | Implemented as deterministic browser simulation; it does not execute learner tests. |
| AI Systems Lab | Structured-output comparison, injection decision, server-reviewed model-strategy tradeoff, context-window assembly, human-approved tool workflow, a server-scored 20-case evaluation dataset, and Debug Bay with 12 distinct failure classes | Partial: model cost/latency values are fictional instructional constraints, not live provider telemetry; no live model execution until a controlled provider is configured. |
| Interview Arena | Timed 30/45/60/90-minute prototype simulations, shorter drills, guided/limited/no-hint mode, follow-up pressure questions, a two-round changing-requirements exercise, server-owned evidence rubric, and local response replay | Partial: no voice transcription, code replay, or dynamic live coach. |
...
| Versioned source-of-truth | Source registry includes publisher, canonical URL, version, locator, claim, verified/revalidation dates, hierarchy type, deprecation status/replacement metadata, review status, individually authored per-concept definition/application/limitation/assessment mappings, a source-health publication report, and mode-specific primary-source disclosures within Systems Practice | Partial: automatic scheduled revalidation and independent technical approval remain to be added. |
| Isolated code execution | Server-side `ExecutionProvider` contract validates bounded requests; an explicit HTTPS-only, security-approved configuration gate protects activation; unconfigured provider refuses execution | Partial by design: no sandbox provider is configured and the main web process never executes learner code. The documented provider-launch review is still required before activation. |
