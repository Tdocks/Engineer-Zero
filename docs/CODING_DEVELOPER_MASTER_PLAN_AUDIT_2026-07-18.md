...
| Retrieval, spacing, interleaving, self-explanation, specific feedback | Protected 12-prompt mixed retrieval forms, due-review list, comprehension gates, targeted responses | Partial: delayed review schedule exists but not yet scheduled notifications or adaptive sequencing. |
...
| Code Lab | Editor, structural review, comprehension record, local-project handoff | Partial: execution/test output remains local by design. |
...
| Assessment engine | Server-only answer keys, randomized mixed forms, response feedback, local attempt records | Partial: response review is deterministic and should not certify independent coding skill. |
...
| Gamification | Six XP categories, challenge feedback, local mastery display | Partial: badges, boss-battle history, and recovery routing remain to be added. |
...
| Versioned source-of-truth | Source registry includes publisher, canonical URL, version, locator, claim, and verified date; instructional research is labeled separately | Partial: per-concept record and automatic revalidation cadence remain to be added. |
...
| AI tutor | Structured local hints and bounded learning flow | Missing: no live provider-backed tutor yet. |
...
- `npm test`: 21 tests pass after the latest instructional-engine, systems-lab, continuation, recovery, and execution-boundary work.
...
2. Move learner evidence, timers, interview attempts, and graduation records to authenticated server-side storage.
3. Add source revalidation dates and per-concept source records rather than only lesson-level source links.
4. Add badges/recovery routing and a verified weighted readiness calculation.
