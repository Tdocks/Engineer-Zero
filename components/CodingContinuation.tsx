...
export function CodingContinuation({ completedIds, onComplete }: { completedIds: string[]; onComplete: (id: string, reflection: string) => void }) {
  ...
  return <section className="coding-continuation">
    ...
    <div className="continuation-layout"><aside>{codingContinuation.map((module) => <button key={module.id} className={module.id === item.id ? "active" : ""} onClick={() => { setSelectedId(module.id); setReflection(""); }}><span>Week {module.week}</span><b>{module.title}</b>{completedIds.includes(module.id) && <Check size={15} />}</button>)}</aside><article><p className="coding-kicker">WEEK {item.week}</p><h3>{item.title}</h3><p className="continuation-outcome">{item.outcome}</p><h4>Practice sequence</h4><ol>{item.activities.map((activity) => <li key={activity}>{activity}</li>)}</ol><section><span>Required artifact</span><p>{item.artifact}</p></section><section><span>Defense</span><p>{item.defense}</p></section>{!priorWeeksDone && <p className="coding-form-error"><LockKeyhole size={15} /> Complete the earlier continuation evidence first. The content remains visible so you can see the destination.</p>}<label>What did you build, verify, or revise? <textarea value={reflection} disabled={!priorWeeksDone || completed} onChange={(event) => setReflection(event.target.value)} placeholder="Name a concrete artifact, test or evidence, remaining limitation, and next step…" /></label><button className="coding-primary" disabled={!priorWeeksDone || completed || reflection.trim().split(/\s+/).length < 25} onClick={() => onComplete(item.id, reflection)}>{completed ? "Evidence recorded" : "Record continuation evidence"} <Check size={16} /></button></article></div>
  </section>;
}
