...
export type CodingChallenge = {
  ...
  /**
   * Private-ish deterministic study-rubric anchors. These are deliberately
   * concrete claims a learner has to connect in their own explanation; merely
   * repeating a checklist heading is not enough to create reviewable evidence.
   */
...
export const codingChallenges: CodingChallenge[] = [
  { id: "coding-terminal-escape", day: 1, kind: "terminal", title: "Terminal escape room", brief: "Create ai_prototype, enter it, create main.py, and recover from one wrong-folder attempt.", starter: "pwd\nls", requiredSignals: ["mkdir ai_prototype", "cd ai_prototype", "touch main.py"], expectedOutcome: "You can explain where your project lives and safely recover from a path mistake.", comprehensionPrompt: "Explain how you knew you were in the wrong folder and name the command that verified the recovery.", comprehensionRequirements: ["working directory", "recovery command", "why the path matters", "expected file location"], evidenceRequirements: [
    { id: "location", label: "a specific working-directory observation", anchors: ["pwd", "working directory", "location", "path"], minAnchors: 2 },
    { id: "recovery", label: "the exact recovery command and destination", anchors: ["mv", "cd", "main.py", "ai_prototype", "move"], minAnchors: 2 },
    { id: "verification", label: "a verification after recovery", anchors: ["verify", "ls", "pwd", "confirm", "check"], minAnchors: 2 },
    { id: "safety", label: "why the path check prevents a mistake", anchors: ["wrong", "safe", "remove", "path", "folder"], minAnchors: 2 },
  ], competencyWeights: { terminal: 1, testingDebugging: .3 }, sourceIds: ["pythonTutorial"] },
  { id: "coding-triage-cli", day: 1, kind: "code", title: "Equipment status triage CLI", brief: "Write deterministic triage logic that validates input and returns NORMAL, REVIEW, or URGENT.", starter: "def evaluate_reading(temperature: float) -> str:\n    # return a risk level\n    pass\n", requiredSignals: ["def evaluate_reading", "return", "if", "elif"], antiPatterns: ["except Exception", "openai"], expectedOutcome: "A deterministic JSON-ready result with no model dependency.", comprehensionPrompt: "Name the input, the exact threshold boundary, the return value, and why this rule should remain deterministic.", comprehensionRequirements: ["input and output", "threshold boundary", "deterministic rule", "test to run"], evidenceRequirements: [
    { id: "contract", label: "the temperature input and named risk-level output", anchors: ["temperature", "input", "return", "normal", "review", "urgent"], minAnchors: 3 },
    { id: "boundary", label: "an exact threshold or boundary case", anchors: ["90", "80", "threshold", "boundary", "exact"], minAnchors: 2 },
    { id: "determinism", label: "why this remains trusted deterministic logic", anchors: ["deterministic", "rule", "repeatable", "model", "ai"], minAnchors: 2 },
    { id: "verification", label: "a specific test or invalid-input check", anchors: ["test", "assert", "invalid", "valueerror", "verify"], minAnchors: 2 },
  ], competencyWeights: { python: 1, dataInterfaces: .6, decomposition: .6, testingDebugging: .4 }, sourceIds: ["pythonTutorial"] },
  { id: "coding-triage-api", day: 2, kind: "api", title: "Equipment Triage API", brief: "Define a typed POST /triage endpoint and keep threshold logic in a service function.", starter: "from fastapi import FastAPI\nfrom pydantic import BaseModel\n\napp = FastAPI()\n\nclass ReadingIn(BaseModel):\n    equipment: str\n    temperature: float\n", requiredSignals: ["FastAPI", "BaseModel", "@app.post", "/triage"], antiPatterns: ["dict:"], expectedOutcome: "A request contract, response contract, and testable business-rule boundary.", comprehensionPrompt: "Explain which layer validates the request, which layer owns the threshold rule, and one failure response the client should receive.", comprehensionRequirements: ["request validation", "service boundary", "failure response", "test to run"], evidenceRequirements: [
    { id: "validation", label: "a typed request-validation boundary", anchors: ["request", "model", "validation", "schema", "input"], minAnchors: 3 },
    { id: "service", label: "a separation between route and deterministic service logic", anchors: ["route", "service", "function", "threshold", "business"], minAnchors: 3 },
    { id: "failure", label: "a client-visible invalid-input response", anchors: ["invalid", "422", "error", "client", "response"], minAnchors: 2 },
    { id: "test", label: "a direct service or HTTP contract test", anchors: ["test", "assert", "http", "boundary", "verify"], minAnchors: 2 },
  ], competencyWeights: { api: 1, dataInterfaces: .7, python: .5, testingDebugging: .5 }, sourceIds: ["fastapiBody", "fastapiResponse"] },
  { id: "coding-test-repair", day: 2, kind: "debug", title: "Test before fix", brief: "Repair a threshold test that misses the exact 90-degree boundary.", starter: "def test_hot_reading_is_urgent():\n    assert evaluate_reading(91) == \"URGENT\"\n", requiredSignals: ["90", "assert"], expectedOutcome: "A boundary test that makes the requirement visible.", comprehensionPrompt: "State the requirement the old test missed, why 91 is insufficient evidence, and what regression your revised test prevents.", comprehensionRequirements: ["boundary requirement", "why 91 is insufficient", "regression", "test assertion"], evidenceRequirements: [
    { id: "requirement", label: "the exact 90-degree requirement", anchors: ["90", "threshold", "urgent", "exact", "boundary"], minAnchors: 2 },
    { id: "gap", label: "why a 91-degree-only test is insufficient", anchors: ["91", "miss", "not", "boundary", "insufficient"], minAnchors: 2 },
    { id: "assertion", label: "a concrete revised assertion", anchors: ["assert", "evaluate_reading", "urgent", "90", "test"], minAnchors: 3 },
    { id: "regression", label: "the regression that the test prevents", anchors: ["regression", "change", "rule", "protect", "prevent"], minAnchors: 2 },
  ], competencyWeights: { testingDebugging: 1, python: .5, defense: .4 }, sourceIds: ["pytest"] },
  { id: "coding-ai-extraction", day: 3, kind: "ai", title: "AI-assisted maintenance extraction", brief: "Design a structured extraction boundary; deterministic code must retain the final risk decision.", starter: "class Extraction(BaseModel):\n    equipment: str | None = None\n    observations: list[str]\n    uncertainties: list[str]\n", requiredSignals: ["uncertainties", "observations", "BaseModel"], antiPatterns: ["execute_action", "api_key = \""], expectedOutcome: "A schema-first draft that can be validated, escalated, or rejected.", comprehensionPrompt: "Identify the model’s narrow task, the trusted validation step, one uncertainty case, and the human decision that remains outside the model.", comprehensionRequirements: ["model boundary", "trusted validation", "uncertainty", "human approval"], evidenceRequirements: [
    { id: "model", label: "the model's bounded extraction task", anchors: ["model", "extract", "draft", "unstructured", "notes"], minAnchors: 2 },
    { id: "validation", label: "schema validation before trusted policy", anchors: ["schema", "validation", "trusted", "structured", "reject"], minAnchors: 3 },
    { id: "uncertainty", label: "a concrete uncertainty or missing-fact path", anchors: ["uncertainty", "missing", "unit", "ambiguous", "escalate"], minAnchors: 2 },
    { id: "approval", label: "human approval outside the model boundary", anchors: ["human", "approve", "action", "risk", "decision"], minAnchors: 2 },
  ], competencyWeights: { aiApplications: 1, securityReliability: .8, api: .4, defense: .5 }, sourceIds: ["openaiStructured", "nistAiRmf", "owaspGenAi"] },
  { id: "coding-handoff-capstone", day: 4, kind: "defense", title: "Mission Operations Handoff Assistant", brief: "Scope, design, test, and defend a small handoff API with human review and degraded mode.", starter: "POST /handoffs\nInput: free-text notes\nOutput: issues, owners, urgency, review_status, uncertainties\n", requiredSignals: ["review", "test", "fallback", "validation"], expectedOutcome: "A bounded prototype design that another engineer can run, test, and challenge.", comprehensionPrompt: "Give the user outcome, the write/approval boundary, one end-to-end test, degraded behavior when AI is unavailable, and the first production concern you would escalate.", comprehensionRequirements: ["user outcome", "approval boundary", "end-to-end test", "degraded mode", "production escalation"], evidenceRequirements: [
    { id: "outcome", label: "the user outcome and bounded scope", anchors: ["handoff", "user", "manual", "outcome", "scope"], minAnchors: 2 },
    { id: "approval", label: "an explicit write or human-approval boundary", anchors: ["human", "approve", "review", "write", "action"], minAnchors: 2 },
    { id: "test", label: "an end-to-end contract test", anchors: ["test", "post", "handoffs", "assert", "response"], minAnchors: 3 },
    { id: "fallback", label: "a degraded model-unavailable path", anchors: ["model", "unavailable", "fallback", "degraded", "failure"], minAnchors: 2 },
    { id: "escalation", label: "a specific next production concern or owner", anchors: ["production", "security", "identity", "escalate", "next"], minAnchors: 2 },
  ], competencyWeights: { decomposition: 1, api: .8, aiApplications: .8, testingDebugging: .7, defense: 1, securityReliability: .7 }, sourceIds: ["fastapiBody", "pytest", "nistSsdf", "nistAiRmf"] },
];
...
export function validateCodingProgram() {
  ...
  for (const item of codingChallenges) {
    ...
    if (!item.brief || !item.starter || !item.requiredSignals.length || !item.expectedOutcome || !item.comprehensionPrompt || !item.comprehensionRequirements.length || item.evidenceRequirements.length < 4) {
